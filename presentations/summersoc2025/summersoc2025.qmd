...
- Decoupled scheduler uses HTTP (`/tasks`) and MQTT (`tasks/+`) for $\theta: T \rightarrow N$
- $T$, $P_{task\_type}(t_i)={S\,(50\%), M\,(30\%), L\,(20\%)}$ and batch size of $200,...,800$
  - $P_{size}(t_i)={10,50,100} \quad P_{payload}(t_i)={10,50,100}$
  - $R_{cpu\_shares}(t_i)={250,500,1000} \quad R_{memory\_size}(t_i)={0.25,0.50,1.0}$
...
**Task states:** <span style="color:gray;"> NONE -> NEW -> </span> SCHEDULED -> OFFLOADED <span style="color:gray;"> -> RUNNING -> COMPLETED</span>
...
**Task states:** <span style="color:gray;"> NONE -> NEW -> SCHEDULED -> OFFLOADED -> </span> RUNNING -> COMPLETED

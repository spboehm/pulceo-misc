...
```{r}
throughput_bar_chart <- ggplot(throughput, aes(x = batchSize, y = throughput, fill = layer)) +
    ...
    labs(
        title = "Bar Chart of Throughput by Batch Size and Layer",
        x = "Batch Size",
...
arrival_rate <- CalculateTaskArrivalRate(tasks, batchSize, layer)
```

```{r}
# Create a bar chart for task arrival rate
arrival_rate_bar_chart <- ggplot(arrival_rate, aes(x = batchSize, y = arrival_rate, fill = layer)) +
    geom_bar(stat = "identity", position = "dodge", color = "darkblue") +
    labs(
        title = "Bar Chart of Task Arrival Rate by Batch Size and Layer",
        x = "Batch Size",

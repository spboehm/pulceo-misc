...
if (exists("params")) {
    SUBFOLDER <- ifelse(params$subfolder != "", params$subfolder, "test")
} else {
    SUBFOLDER <- "summersoc2025"
}
```
...
#     ggsave(paste0("plots/", SUBFOLDER, "/", filename, ".svg"), plot + theme(plot.margin = margin(0, 0, 0, 0, "pt")), width = width, height = height, device = "svg")
# }
```
...
# Create individual boxplots using the function
...
#boxplot_scheduling <- CreateTaskMetricBoxplot(scheduling_time_df, "Offloading Time")
#boxplot_processing <- CreateTaskMetricBoxplot(processing_time_df, "Processing Time (Application)")
#boxplot_completion <- CreateTaskMetricBoxplot(completion_time_df, "Completion Time (Total)")

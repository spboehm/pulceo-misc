...
- `Memory_Capacitiy`: The available memory is required in MB, GB, or TB. Example: 5 GB.
...
- `Network Capacity`: The available network resources are required. This is usually the available bandwidth provided by the network interfaces of a particular node. Example: 1000 Mbps download/upload.

```{r}
# static node properties
sNodeProps <- read.csv2("csv/nodes-static-properties.csv")
```
...
- `Disk_Util`: Represents the current disk utilization in terms of storage. Example: 0.35. This is **not** I/O utilization.
...
```{r}
# dynamic node properties
dNodeProps<- read.csv2("csv/nodes-dynamic-properties.csv")
```
